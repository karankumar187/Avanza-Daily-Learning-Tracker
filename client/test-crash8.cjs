...
(async () => {
  ...
  if (errText.includes('TypeError') || errText.includes('Error')) {
      const match = errText.match(/class="[^"]*error[^"]*">([\s\S]*?)<\/div>/i);
      console.log("Error boundary generic match?", match ? match[1] : 'No');
  }
